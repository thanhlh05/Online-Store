Feature('TC_FE_EMP_10 - Cancelled order shows only Show action');

Scenario('Cancelled order cannot be finished or cancelled again', async ({ I }) => {
  ...
  I.usePlaywrightTo('cancel own order and verify only Show action remains', async ({ page }) => {
    ...
    await page.evaluate(id => {
      ...
      const a = [...row.querySelectorAll('a')].find(x => x.textContent.trim() === 'Cancel');
      if (!a) throw new Error(`Cancel link not found in row ${id}`);
      a.click();
    }, orderId);
    ...
    if (JSON.stringify(actions) !== JSON.stringify(['Show'])) {
      throw new Error(`Unexpected action links for cancelled order: ${actions}`);
    }
  });
});