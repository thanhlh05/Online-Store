...
2. **`Add New Product`**
   ...
   - **Lưu ý:** Trong Body, đổi `productId` thành một mã hoàn toàn mới (VD: `NEW_001`) để tránh lỗi 500.
   - **Kỳ vọng:** `200 OK`.
...
3. **`Add a product to the cart`**
   - **Action:** Thêm mã `NEW_001` (hoặc `B0001`) vào giỏ.
   - **Kỳ vọng:** `200 OK`.
4. **`Update quantity`**
...
6. **`Remove product from the cart`**
   - **Action:** Bỏ mã `F0001` ra khỏi giỏ, giữ lại `NEW_001` để thanh toán.
...
4. **`View order details`**
   - **Action:** Sửa URL thành `/order/{id_đơn_1}` (VD: `/order/20`).
   - **Kỳ vọng:** Hiển thị chi tiết đơn.
5. **`Cancel order`**
   - **Action:** Sửa URL thành `/order/cancel/{id_đơn_1}` (VD: `/order/cancel/20`).
...
2. **`Complete order`**
   - **Action:** Sửa URL thành `/order/finish/{id_đơn_2}` (VD: `/order/finish/21`).
